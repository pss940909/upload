...
function updateCartList() {
  let cart_list_temp = "";
  let cart_total = 0;
  const cart_subtotal = document.querySelector(".cart_subtotal");
  ...
  cart.forEach(function (item) {
    cart_list_temp += ` <tr>
        ...
        <td>NT$${item.price}</td>
        <td class='qty_td'><a href="#" class='minusQty' data-id=${item.id}>-</a>
        <a class="qty">${item.quantity}</a>
        <a href="#" class='addQty' data-id=${item.id}>+</a></td>
        <td>${item.price * item.quantity}</td>
        <td class="discardBtn">
        <i class="fa-solid fa-trash-can deleteBtn" data-id=${item.id}></i>
        </td>
  ...
  cart_list.innerHTML = cart_list_temp;
  cart_subtotal.textContent = cart_total;
}
updateCartList();
